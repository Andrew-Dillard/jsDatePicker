...
function parse(dirtyDateString, dirtyFormatString, dirtyReferenceDate, options) {
    ...
    var _loop = function(_token) {
        if (!(options !== null && options !== void 0 && options.useAdditionalWeekYearTokens) && (0, _indexJs6.isProtectedWeekYearToken)(_token)) (0, _indexJs6.throwProtectedError)(_token, formatString, dirtyDateString);
        if (!(options !== null && options !== void 0 && options.useAdditionalDayOfYearTokens) && (0, _indexJs6.isProtectedDayOfYearToken)(_token)) (0, _indexJs6.throwProtectedError)(_token, formatString, dirtyDateString);
        ...
        if (parser) {
            ...
            if (Array.isArray(incompatibleTokens)) {
                ...
                if (incompatibleToken) throw new RangeError("The format string mustn't contain `".concat(incompatibleToken.fullToken, "` and `").concat(_token, "` at the same time"));
            } else if (parser.incompatibleTokens === "*" && usedTokens.length > 0) throw new RangeError("The format string mustn't contain `".concat(_token, "` and any other token at the same time"));
            usedTokens.push({
            ...
            if (firstCharacter.match(unescapedLatinCharacterRegExp)) throw new RangeError("Format string contains an unescaped latin alphabet character `" + firstCharacter + "`");
             // Replace two single quote characters with one single quote character
            if (_token === "''") _token = "'";
            else if (firstCharacter === "'") _token = cleanEscapedString(_token);
             // Cut token from string, or, if string doesn't match the token, return Invalid Date
            if (dateString.indexOf(_token) === 0) dateString = dateString.slice(_token.length);
            else {
                token = _token;
                return {
                    v: new Date(NaN)
...
},{"../_lib/requiredArgs/index.js":"9wUgQ","../constants/index.js":"iOhcx","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["2mNKm","6rimH"], "6rimH", "parcelRequire94c2")
